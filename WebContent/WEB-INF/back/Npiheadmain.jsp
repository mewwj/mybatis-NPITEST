...
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
  ...
  <legend style="margin-left:7cm;">专案信息展示</legend>
           ...
           <form action="/npi.test/HEAD_ToHeadMain"  name="form1" id="form1" method="post" >
        	...
        	客户名称：<input type="text" name="zcusname" />
        	专案名称：<input type="text" name="zproname" />
        	阶段选择：<select name="zprovince" id="zprovince"  style="width: 100px;" onchange="changeSelect(this.selectedIndex)">
		...
		<div class="layui-btn-group" style="margin-left: 5cm;">
		  <a class="layui-btn" href="/npi.test/HEAD_ToHeadInsert" >增加</a>
		</div>
		<!-- 表格 -->
	  ...
	  <c:forEach items="${mainheadlist}" var="item" varStatus="status"> 
	  	...
	  	<td>${item.goodrate}</td>
		  	<td>
			  <a href="/npi.test/HEAD_DoHeadMain?iid=${item.id}">添加检讨信息</a> 
			  <a href="/npi.test/ITEM_ToItemsMain?zid=${item.id}">查看检讨信息</a> 
		    <td>
	  	</tr>
	    </c:forEach>
	  </tbody>
